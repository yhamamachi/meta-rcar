...
do_install_append() {
    if ${@bb.utils.contains('MACHINE_FEATURES', 'videobox_v2', 'true', 'false', d)}; then
        if ${@bb.utils.contains('DISTRO_FEATURES', 'videobox-recorder', 'true', 'false', d)}; then
            install -m 0644 ${WORKDIR}/videobox_v2/fstab_ro ${D}/${sysconfdir}/
        fi
        install -m 0644 ${WORKDIR}/videobox_v2/fstab ${D}/${sysconfdir}/
    fi
    if ${@bb.utils.contains('MACHINE_FEATURES', 'videobox', 'true', 'false', d)}; then
        if ${@bb.utils.contains('DISTRO_FEATURES', 'videobox-recorder', 'true', 'false', d)}; then
            install -m 0644 ${WORKDIR}/videobox/fstab_ro ${D}/${sysconfdir}/
        fi
        install -m 0644 ${WORKDIR}/videobox/fstab ${D}/${sysconfdir}/
    fi
